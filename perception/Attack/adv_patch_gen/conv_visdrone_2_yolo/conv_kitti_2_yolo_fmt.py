...
CLASS_2_CONSIDER = {0, 1, 2, 3, 4, 5}
CLASS_2_ID = {'Car':0, 'Truck':1, 'Bus':2, 'Motorcycle':3, 'Bicycle':4, 'Pedestrian':5}
CLASS_ID_REMAP = None
IMG_EXT = {".jpg", ".png"}
            ...
            if True:
                ...
                with open(src_annot_file, "r") as fr, open(target_annot_file, "w") as fw:
                    for coords in fr:
                        ...
                        annots = list(map(float, coords.strip().strip(",").split(" ")[1:]))
                        x1, y1 = annots[3], annots[4]
                        x2, y2 = annots[5], annots[6]
                        ...
                        # class_id= annots[1]
                        class_id = CLASS_2_ID[coords.strip().strip(",").split(" ")[0]]
                        ...
                        #     new_box_count += 1
                        class_id = CLASS_ID_REMAP[class_id] if CLASS_ID_REMAP else class_id
                        fw.write(f"{int(class_id)} {xc/iw} {yc/ih} {w/iw} {h/ih}\n")
                        new_box_count += 1
