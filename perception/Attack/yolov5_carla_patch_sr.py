...
vw = cv2.VideoWriter("out.mp4", fourcc, 60, (1280, 1280))
#构造相机投影矩阵函数
...
class CarlaPatchAttacker():
    ...
    # 初始化
    def init_attacker(self, client_port=2000):
    ...
    # 生成车辆
    def generate_vihecle(self, tm_port=8000, vehicle_type='vehicle.lincoln.mkz_2020'):
    ...
    def render(self, image_shape=(640, 640), apply_patch_transforms=False):
        ...
        while True:
            ...
            #变换patch并增加至原图
            zeros_tensor = torch.zeros([1, 5]).to('cuda')
            img_fake_batch = img
            lab_fake_batch = label.unsqueeze(0).to('cuda')
            if len(lab_fake_batch[0]) == 1 and torch.equal(lab_fake_batch[0], zeros_tensor):
                p_tensor_batch = img        # 无检测目标则使用原图
            else:
