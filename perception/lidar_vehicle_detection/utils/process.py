...
import torch
import pdb
from lidar_vehicle_detection.ops.iou3d_module import boxes_overlap_bev, boxes_iou_bev
...
def iou3d(bboxes1, bboxes2):
    ...
    bev_overlap = boxes_overlap_bev(bboxes1_bev.cuda(), bboxes2_bev.cuda()) # (n, m)

    # 3. overlap and volume
    overlap = height_overlap * bev_overlap.cpu()
...
def iou3d_camera(bboxes1, bboxes2):
    ...
    bboxes1_bottom = bboxes1[:, 1] - bboxes1[:, 4]
    bboxes2_bottom = bboxes2[:, 1] - bboxes2[:, 4]
    ...
    # 3. overlap and volume
    # overlap = height_overlap.cuda() * bev_overlap
    overlap = height_overlap * bev_overlap.cpu()
    volume1 = bboxes1[:, 3] * bboxes1[:, 4] * bboxes1[:, 5]
    volume2 = bboxes2[:, 3] * bboxes2[:, 4] * bboxes2[:, 5]
    volume = volume1[:, None] + volume2[None, :] # (n, m)
    ...
    # 4. iou
    # iou = overlap / (volume.cuda() - overlap + 1e-8)
    iou = overlap / (volume - overlap + 1e-8)
