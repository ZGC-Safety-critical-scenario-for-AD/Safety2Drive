...
class PillarEncoder(nn.Module):
    ...
    def forward(self, pillars, coors_batch, npoints_per_pillar):
        ...
        # print(features)
        # print(torch.sum(pooling_features))
...
class PointPillars(nn.Module):
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None, pillars=None, coors_batch=None, npoints_per_pillar=None, adv = False):
        batch_size = len(batched_pts)
        # batched_pts: list[tensor] -> pillars: (p1 + p2 + ... + pb, num_points, c), 
        #                              coors_batch: (p1 + p2 + ... + pb, 1 + 3), 
        #                              num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch size)
        # print(batched_pts.requires_grad)
        if not adv:
            pillars, coors_batch, npoints_per_pillar = self.pillar_layer(batched_pts)
        # print(pillars.requires_grad)
        # print(pillars, coors_batch, npoints_per_pillar)
        ...
        # print(pillar_features.requires_grad)
        # print(pillar_features)
        # print(torch.sum(pillar_features))
