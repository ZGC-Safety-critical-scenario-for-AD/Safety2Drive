...
class LidarVehicleDetector():
    ...
    def get_vehicles(self, img, pc, P2, R0_rect, Tr_velo_to_cam, gt_lidar_bboxes, gt_labels, adv=True, epsilon=0.05, steps = 20):
        ...
        perturb = torch.randn_like(pillars)
        ...
        for i in range(steps):
            ...
            # print(loss_dict)
            loss = loss_dict['total_loss'] + torch.norm(perturb)
            self.model.zero_grad()
            loss.backward()
            print(batched_pts)
            # print(pillars.grad.data)
            perturb_grad = perturb.grad.data
            # pillars_grad_sign = pillars_grad.sign()
            perturb_n = perturb - epsilon * perturb_grad
            perturb = perturb_n.detach()
