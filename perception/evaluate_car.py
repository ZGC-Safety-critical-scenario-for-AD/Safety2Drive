...
from lidar_vehicle_detection.utils import setup_seed, keep_bbox_from_image_range, \
    keep_bbox_from_lidar_range, write_pickle, write_label, \
    iou2d, iou3d_camera, iou_bev, iou3d
# from dataset import Kitti, get_dataloader, Kitti_car
...
def do_eval(det_results, gt_results, CLASSES, saved_path, exp_name):
    '''
    det_results: list,
    ...
    f = open(os.path.join(saved_path, f'{exp_name}.txt'), 'w')
    ...
    for i, id in enumerate(ids):
        ...
        # 1.1, 2d bboxes iou
        gt_result['bbox'] = np.array(gt_result['bbox'])
        gt_bboxes2d = np.array(gt_result['bbox']).astype(np.float32)
        det_result['bbox'] = np.array(det_result['bbox'])
        det_bboxes2d = np.array(det_result['bbox']).astype(np.float32)
        ...
        # 1.2, bev iou
        gt_result['location'] = np.array(gt_result['location'])
        gt_result['dimensions'] = np.array(gt_result['dimensions'])
        gt_result['rotation_y'] = np.array(gt_result['rotation_y'])
        det_result['location'] = np.array(det_result['location'])
        det_result['dimensions'] = np.array(det_result['dimensions'])
        det_result['rotation_y'] = np.array(det_result['rotation_y'])
        
        gt_location = np.array(gt_result['location']).astype(np.float32)
        gt_dimensions = np.array(gt_result['dimensions']).astype(np.float32)
        gt_rotation_y = np.array(gt_result['rotation_y']).astype(np.float32)
        det_location = np.array(det_result['location']).astype(np.float32)
        det_dimensions = np.array(det_result['dimensions']).astype(np.float32)
        det_rotation_y = np.array(det_result['rotation_y']).astype(np.float32)
        ...
        #     print(det_bev.shape)
        # # try:
        # iou_bev_v = iou_bev(torch.from_numpy(gt_bev).reshape(-1, 5).cuda(), torch.from_numpy(det_bev).reshape(-1, 5).cuda())
        # # except:
        # #     print(i)
        # #     print(det_result)
        ...
        # 1.3, 3dbboxes iou
        # print(gt_location.shape, gt_dimensions.shape, gt_rotation_y[:, None].shape)
        ...
        print(1, gt_bboxes3d)
        print(2, det_bboxes3d)
        # iou3d_v = iou3d_camera(torch.from_numpy(gt_bboxes3d).reshape(-1, 7), torch.from_numpy(det_bboxes3d).reshape(-1, 7))
        iou3d_v = iou3d_camera(torch.from_numpy(gt_bboxes3d).reshape(-1, 7), torch.from_numpy(det_bboxes3d).reshape(-1, 7))
    ...
    for e_ind, eval_type in enumerate(['bbox_2d', 'bbox_3d']):
        ...
        for cls in CLASSES:
            ...
            for difficulty in [0, 1, 2]:
                ...
                for id in ids:
                    ...
                    # print(det_result['bbox'].shape)
                    try:
                        cur_det_heights = det_result['bbox'][:, 3] - det_result['bbox'][:, 1]
                    except:
                        cur_det_heights = [0 for i in range(len(cur_det_names))]
                    det_ignores = []
                    # print(len(cur_det_names))
                    # print(cur_det_heights)
...
def main(args):
    val_dataset = Kitti_car(data_root=args.data_root,
                        split='val')
    val_dataloader = get_dataloader(dataset=val_dataset, 
                                    batch_size=args.batch_size, 
                                    num_workers=args.num_workers,
                                    shuffle=False)
    CLASSES = Kitti_car.CLASSES
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', default='./data/kitti_carla_3city/', 
