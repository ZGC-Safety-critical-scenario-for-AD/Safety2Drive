...
import corruptions_utils.imgaug.augmenters.imgcorruptlike as imgcorruptlike
...
from corruptions_utils.tps_grid_gen import TPSGridGen
...
def points_cam2img(points_3d, proj_mat):
    ...
    if d1 == 3:
        ...
        proj_mat_expanded[:d1, :d2] = torch.from_numpy(proj_mat)
    ...
    points_4 = torch.cat(
        [torch.from_numpy(points_3d), torch.from_numpy(points_3d).new_ones(*points_shape)], dim=-1)
...
class ImagePointAddSun():
    ...
    def sun_sim_point(self, points, lidar2img, severity, watch_img=False, file_path='', temp_dict_trans_information={}):
        ...
        if not (mainflare_mask==0).all():
            ...
            pts_uvd = points_cam2img(points[:,:3], proj_mat=lidar2img)
            pts_2d = pts_uvd[:,:2]
            pts_depth = pts_uvd[:,2]
...
class ImageAddGaussianNoise():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            ...
            imgcorruptlike.GaussianNoise(severity=severity, seed=seed),
        ])
...
class ImageAddImpulseNoise():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            ...
            imgcorruptlike.ImpulseNoise(severity=severity, seed=seed),
        ])
...
class ImageAddSnow():
    def __init__(self, severity, seed) -> None:
        self.iaa_seq = iaa.Sequential([
            ...
            imgcorruptlike.Snow(severity=severity, seed=seed),
        ])
